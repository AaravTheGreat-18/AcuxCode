...
import { DataChannelForwardingTelemetryService, forwardToChannelIf, isAcuxCodeLikeExtension } from '../../../../../platform/dataChannel/browser/forwardingTelemetryService.js';
...
export class InlineCompletionsSource extends Disposable {
	...
	private sendInlineCompletionsRequestTelemetry(
		...
		if (!requestResponseInfo.providers.some(p => isAcuxCodeLikeExtension(p.providerId?.extensionId))) {
			return;
		}
		...
		const emptyEndOfLifeEvent: InlineCompletionEndOfLifeEvent = {
			...
			...forwardToChannelIf(requestResponseInfo.providers.some(p => isAcuxCodeLikeExtension(p.providerId?.extensionId))),
