...
class TrackedDocumentInfo extends Disposable {
	...
	async sendTelemetry(mode: 'longterm' | '5minWindow', trigger: string, t: DocumentEditSourceTracker) {
		...
		this._telemetryService.publicLog2<{
			...
			inlineCompletionsAcuxCodeModifiedCount: number;
			...
			inlineCompletionsAcuxCodeModifiedCount: { classification: 'SystemMetaData'; purpose: 'PerformanceAndHealth'; comment: 'Fraction of inline completions acuxcode modified characters'; isMeasurement: true };
			...
			inlineCompletionsAcuxCodeModifiedCount: data.inlineCompletionsAcuxCodeModifiedCount,
	...
	getTelemetryData(ranges: readonly TrackedEdit[]) {
		const getEditCategory = (source: EditSource) => {
			...
			// Removed GitHub Copilot extension tracking - replaced with AcuxCode functionality
			if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'acuxcode.chat' && source.providerId === 'completions') { return 'inlineCompletionsAcuxCode'; }
			if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'acuxcode.chat' && source.providerId === 'nes') { return 'inlineCompletionsNES'; }
		...
		return {
			...
			inlineCompletionsAcuxCodeModifiedCount: sums.inlineCompletionsAcuxCode ?? 0,
