...
export class ComputeAutomaticInstructions {
	...
	public async hasAgentInstructions(token: CancellationToken): Promise<boolean> {
		const useCopilotInstructionsFiles = this._configurationService.getValue(PromptsConfig.USE_ACUXCODE_INSTRUCTION_FILES);
	...
	private async _addAgentInstructions(variables: ChatRequestVariableSet, telemetryEvent: InstructionsCollectionEvent, token: CancellationToken): Promise<void> {
		...
		if (useCopilotInstructionsFiles) {
			for (const folder of folders) {
				const file = joinPath(folder.uri, `.github/` + COPILOT_CUSTOM_INSTRUCTIONS_FILENAME);
				if (await this._fileService.exists(file)) {
					entries.add(toPromptFileVariableEntry(file, PromptFileVariableKind.Instruction, localize('instruction.file.reason.copilot', 'Automatically attached as setting {0} is enabled', PromptsConfig.USE_ACUXCODE_INSTRUCTION_FILES), true));
