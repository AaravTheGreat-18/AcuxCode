...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	private getWelcomeViewContent(additionalMessage: string | IMarkdownString | undefined, expEmptyState?: boolean): IChatViewWelcomeContent {
		...
		if (this.isLockedToCodingAgent) {
			...
			const message = new MarkdownString(localize('genericCodingAgentMessage', "This chat session will be forwarded to the {0} coding agent where work is completed in the background. ", this._codingAgentPrefix) + this.chatDisclaimer);
		...
		if (this.input.currentModeKind === ChatModeKind.Ask) {
			...
			const agentHelpMessage = localize('agentMessage', "Ask to edit your files in [agent mode]({0}). Agent mode will automatically use multiple requests to pick files to edit, run terminal commands, and iterate on errors.", 'https://aka.ms/vscode-acuxcode-agent');
	...
	private getNewWelcomeViewContent(): IChatViewWelcomeContent {
		...
		if (this.chatEntitlementService.anonymous) {
			additionalMessage = new MarkdownString(localize({ key: 'settings', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "AI responses may be inaccurate.\nBy continuing with {0} AcuxCode, you agree to {1}'s [Terms]({2}) and [Privacy Statement]({3}).", defaultChat.provider.default.name, defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl), { isTrusted: true });
		} else {
	 ...
	 * - instructions referenced in the acuxcode settings 'acuxcode-instructions'
