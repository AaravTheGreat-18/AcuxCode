...
import { AcuxCodeChatIcon } from '../chatEditorInput.js';
...
abstract class OpenChatGlobalAction extends Action2 {
	constructor(overrides: Pick<ICommandPaletteOptions, 'keybinding' | 'title' | 'id' | 'menu'>, private readonly mode?: IChatMode) {
		super({
			...overrides,
			icon: AcuxCodeChatIcon,  // Keep ThemeIcon for command palette
			f1: true,
...
export function registerChatActions() {
	...
	// Removed Copilot-specific manage settings action
	
	// Removed Copilot extensions search action
	
	registerAction2(class ConfigureAcuxCodeCompletions extends Action2 {

		constructor() {
			super({
				id: 'workbench.action.chat.configureCodeCompletions',
				title: localize2('configureCompletions', "Configure AcuxCode AI Completions..."),
				precondition: ContextKeyExpr.and(
	...
	registerAction2(class ShowQuotaExceededDialogAction extends Action2 {
		...
		constructor() {
			super({
				...
				title: localize('upgradeChat', "Upgrade AcuxCode AI Plan")
			});
		}
		...
		override async run(accessor: ServicesAccessor) {
			...
			const upgradeToPro = free ? localize('upgradeToPro', "Upgrade to AcuxCode AI Pro for:\n- Unlimited code completions\n- Unlimited chat messages\n- Access to premium models") : undefined;

			await dialogService.prompt({
				type: 'none',
				message: localize('acuxcodeQuotaReached', "AcuxCode AI Quota Reached"),
				cancelButton: {
				...
				buttons: [
					{
						label: free ? localize('upgradePro', "Upgrade to AcuxCode AI Pro") : localize('upgradePlan', "Upgrade AcuxCode AI Plan"),
						run: () => {
	...
	registerAction2(class UpdateInstructionsAction extends Action2 {
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			const query = `Analyze this codebase to generate or update \`.acuxcode/ai-instructions.md\` for guiding AI coding agents.
...
Source existing AI conventions from \`**/{.acuxcode/ai-instructions.md,AGENT.md,AGENTS.md,.cursorrules,.windsurfrules,.clinerules,.cursor/rules/**,.windsurf/rules/**,.clinerules/**,README.md}\` (do one glob search).

Guidelines:
- If \`.acuxcode/ai-instructions.md\` exists, merge intelligently - preserve valuable content while updating outdated sections
...
Update \`.acuxcode/ai-instructions.md\` for the user, then ask for feedback on any unclear or incomplete sections to iterate.`;
...
registerAction2(class ToggleAcuxCodeControl extends ToggleTitleBarConfigAction {
	constructor() {
		super(
			'chat.commandCenter.enabled',
			localize('toggle.chatControl', 'AcuxCode AI Chat Controls'),
			localize('toggle.chatControlsDescription', "Toggle visibility of the AcuxCode AI Chat Controls in title bar"), 5,
			ContextKeyExpr.and(
				ContextKeyExpr.and(
					ChatContextKeys.Setup.hidden.negate(),
					ChatContextKeys.Setup.disabled.negate()
...
export class AcuxCodeTitleBarMenuRendering extends Disposable implements IWorkbenchContribution {

	static readonly ID = 'workbench.contrib.acuxcodeTitleBarMenuRendering';
	...
	constructor(
		...
		const disposable = actionViewItemService.register(MenuId.CommandCenter, MenuId.ChatTitleBarMenu, (action, options, instantiationService, windowId) => {
			...
			const dropdownAction = toAction({
				id: 'acuxcode.titleBarMenuRendering.more',
			...
			let primaryActionIcon = AcuxCodeChatIcon;
			if (chatSentiment.installed && !chatSentiment.disabled) {
				if (signedOut && !anonymous) {
					...
					primaryActionTitle = localize('chatQuotaExceededButton', "AcuxCode AI chat quota reached. Click for details.");
...
// TODO@bpasero remove these when Chat extension is built-in
{
	...
	registerGenerateCodeCommand('chat.internal.explain', 'acuxcode.chat.explain');
	registerGenerateCodeCommand('chat.internal.fix', 'acuxcode.chat.fix');
	registerGenerateCodeCommand('chat.internal.review', 'acuxcode.chat.review');
	registerGenerateCodeCommand('chat.internal.generateDocs', 'acuxcode.chat.generateDocs');
	registerGenerateCodeCommand('chat.internal.generateTests', 'acuxcode.chat.generateTests');
