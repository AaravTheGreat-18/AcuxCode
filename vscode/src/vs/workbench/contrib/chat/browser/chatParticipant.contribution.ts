...
export class ChatCompatibilityNotifier extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		// but this is only enabled for AcuxCode Chat now and it needs to be obvious.
	...
	private registerWelcomeView(chatExtension: IExtension) {
		...
		const mainMessage = localize('chatFailErrorMessage', "Chat failed to load because the installed version of the AcuxCode Chat extension is not compatible with this version of {0}. Please ensure that the AcuxCode Chat extension is up to date.", this.productService.nameLong);
		...
		const versionMessage = `AcuxCode Chat version: ${chatExtension.version}`;
