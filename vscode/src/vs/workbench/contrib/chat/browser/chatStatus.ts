...
export class ChatStatusBarEntry extends Disposable implements IWorkbenchContribution {
	...
	private update(): void {
		// Disabled ChatStatusBarEntry - replaced with AcuxCode functionality
		// No longer showing status bar entries
		this.entry?.dispose();
	...
	private getEntryProps(): IStatusbarEntry {
		let text = '$(robot)';
		let ariaLabel = localize('chatStatus', "AcuxCode Status");
		...
		if (isNewUser(this.chatEntitlementService)) {
			...
			if (
				...
				const finishSetup = localize('acuxcodeLaterStatus', "Finish Setup");

				text = `$(robot) ${finishSetup}`;
			...
			if (this.chatEntitlementService.sentiment.disabled || this.chatEntitlementService.sentiment.untrusted) {
				...
				ariaLabel = localize('acuxcodeDisabledStatus', "AcuxCode disabled");
			}

			// Sessions in progress
			else if (chatSessionsInProgressCount > 0) {
				text = '$(robot-in-progress)';
			...
			// Signed out
				...
				const signedOutWarning = localize('notSignedIntoAcuxCode', "Signed out");

				text = `${this.chatEntitlementService.anonymous ? '$(robot)' : '$(robot-not-connected)'} ${signedOutWarning}`;
			...
			// Free Quota Exceeded
				...
				text = `$(robot-warning) ${quotaWarning}`;
		...
		const baseResult = {
			name: localize('chatStatus', "AcuxCode Status"),
			text,
			ariaLabel,
...
function isNewUser(chatEntitlementService: IChatEntitlementService): boolean {
	return !chatEntitlementService.sentiment.installed ||					// acuxcode not installed
		chatEntitlementService.entitlement === ChatEntitlement.Available;	// not yet signed up to acuxcode
}

function canUseAcuxCode(chatEntitlementService: IChatEntitlementService): boolean {
...
class ChatStatusDashboard extends Disposable {
	...
	show(token: CancellationToken): HTMLElement {
		...
		if (chatQuota || completionsQuota || premiumChatQuota) {
			...
			addSeparator(localize('usageTitle', "AcuxCode Usage"), toAction({
				id: 'workbench.action.manageAcuxCode',
			...
			if (this.chatEntitlementService.entitlement === ChatEntitlement.Free && (Number(chatQuota?.percentRemaining) <= 25 || Number(completionsQuota?.percentRemaining) <= 25)) {
				const upgradeProButton = disposables.add(new Button(this.element, { ...defaultButtonStyles, hoverDelegate: nativeHoverDelegate, secondary: canUseAcuxCode(this.chatEntitlementService) /* use secondary color when acuxcode can still be used */ }));
				upgradeProButton.label = localize('upgradeToAcuxCodePro', "Upgrade to AcuxCode Pro");
		...
		// Anonymous Indicator
			...
			addSeparator(localize('anonymousTitle', "AcuxCode Usage"));
		...
		// Completions Snooze
		if (canUseAcuxCode(this.chatEntitlementService)) {
		...
		// New to AcuxCode / Signed out
		{
			...
			if (newUser || signedOut || disabled) {
				...
				if (newUser && anonymousUser) {
					descriptionText = new MarkdownString(localize({ key: 'activeDescriptionAnonymous', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "By continuing with {0} AcuxCode, you agree to {1}'s [Terms]({2}) and [Privacy Statement]({3})", defaultChat.provider.default.name, defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl), { isTrusted: true });
					descriptionClass = `${descriptionClass}.terms`;
				} else if (newUser) {
					descriptionText = localize('activateDescription', "Set up AcuxCode to use AI features.");
				} else if (anonymousUser) {
					descriptionText = localize('enableMoreDescription', "Sign in to enable more AcuxCode AI features.");
				} else if (disabled) {
					descriptionText = localize('enableDescription', "Enable AcuxCode to use AI features.");
				} else {
					descriptionText = localize('signInDescription', "Sign in to use AcuxCode AI features.");
				}

				let buttonLabel: string;
				if (newUser) {
					buttonLabel = localize('activateAcuxCodeButton', "Set up AcuxCode");
				} else if (anonymousUser) {
					buttonLabel = localize('enableMoreAcuxCodeButton', "Enable more AI Features");
				} else if (disabled) {
					buttonLabel = localize('enableAcuxCodeButton', "Enable AcuxCode");
				} else {
					buttonLabel = localize('signInToUseAcuxCodeButton', "Sign in to use AcuxCode");
				}
	...
	private createNextEditSuggestionsSetting(container: HTMLElement, label: string, completionsSettingAccessor: ISettingsAccessor, disposables: DisposableStore): void {
		...
		disposables.add(this.configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration(completionsSettingId)) {
				if (completionsSettingAccessor.readSetting() && canUseAcuxCode(this.chatEntitlementService)) {
					checkbox.enable();
