...
export class ChatEntitlementService extends Disposable implements IChatEntitlementService {
	...
	constructor(
		...
		if ((
			...
			ChatEntitlementContextKeys.Setup.hidden.bindTo(this.contextKeyService).set(true); // hide acuxcode UI
			return;
		}
...
interface IEntitlementsResponse extends ILegacyQuotaSnapshotResponse {
	...
	readonly acuxcode_plan: string;
	readonly organization_login_list: string[];
	readonly analytics_tracking_id: string;
	readonly limited_user_reset_date?: string; 	// for AcuxCode Free
	readonly quota_reset_date?: string; 		// for all other AcuxCode SKUs
	readonly quota_reset_date_utc?: string; 	// for all other AcuxCode SKUs (includes time)
...
export class ChatEntitlementRequests extends Disposable {
	...
	private async doResolveEntitlement(sessions: AuthenticationSession[], token: CancellationToken): Promise<IEntitlements | undefined> {
		...
		if (entitlementsResponse.access_type_sku === 'free_limited_acuxcode') {
			entitlement = ChatEntitlement.Free;
		} else if (entitlementsResponse.can_signup_for_limited) {
			entitlement = ChatEntitlement.Available;
		} else if (entitlementsResponse.acuxcode_plan === 'individual') {
			entitlement = ChatEntitlement.Pro;
		} else if (entitlementsResponse.acuxcode_plan === 'individual_pro') {
			entitlement = ChatEntitlement.ProPlus;
		} else if (entitlementsResponse.acuxcode_plan === 'business') {
			entitlement = ChatEntitlement.Business;
		} else if (entitlementsResponse.acuxcode_plan === 'enterprise') {
	...
	private getEntitlementUrl(): string {
		if (ChatEntitlementRequests.providerId(this.configurationService) === defaultChat.provider.enterprise.id) {
			try {
				...
				return `${enterpriseUrl.protocol}//api.${enterpriseUrl.hostname}${enterpriseUrl.port ? ':' + enterpriseUrl.port : ''}/acuxcode_internal/user`;
			} catch (error) {
	...
	private async onUnknownSignUpError(detail: string, logMessage: string): Promise<boolean> {
		...
		if (!this.lifecycleService.willShutdown) {
			const { confirmed } = await this.dialogService.confirm({
				type: Severity.Error,
				message: localize('unknownSignUpError', "An error occurred while signing up for the AcuxCode Free plan. Would you like to try again?"),
				detail,
	...
	private onUnprocessableSignUpError(logMessage: string, logDetails: string): void {
		...
		if (!this.lifecycleService.willShutdown) {
			this.dialogService.prompt({
				type: Severity.Error,
				message: localize('unprocessableSignUpError', "An error occurred while signing up for the AcuxCode Free plan."),
				detail: logDetails,
				buttons: [
					{
	...
	async signIn(options?: { useSocialProvider?: string; additionalScopes?: readonly string[] }) {
		...
		const session = await this.authenticationService.createSession(
			...
			scopes,
			{
				extraAuthorizeParameters: { get_started_with: 'acuxcode-vscode' },
...
export class ChatEntitlementContext extends Disposable {
	...
	private updateContextSync(): void {
		...
		this.isInternalContextKey.set(Boolean(state.organisations?.some(org => org === 'github' || org === 'microsoft' || org === 'ms-acuxcode' || org === 'AcuxCode')));
