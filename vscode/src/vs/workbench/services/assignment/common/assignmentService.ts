...
import { AcuxCodeAssignmentFilterProvider } from './assignmentFilters.js';
...
export class WorkbenchAssignmentService extends Disposable implements IAssignmentService {
	...
	private async setupTASClient(): Promise<TASClient> {
		...
		const extensionsFilterProvider = this.instantiationService.createInstance(AcuxCodeAssignmentFilterProvider);
